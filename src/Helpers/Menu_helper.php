    ...
    function afficher_menu_admin($parent, $niveau, $array, $table = 'tabs')
    {
        ...
        foreach ($array as $noeud) {
            ...
            if ($parent == $noeud->id_parent) {
                ...
                $html .= "<li class=\"dd-item " . $custom . " dd-item-active-$noeud->active\" data-method=\"$noeud->slug\" data-id=\"$noeud->id_menu\" data-niveau=\"$niveau\" data-niveau_precedent=\"$niveau_precedent\" data-parent=\"$parent\">";
                ...
                $html .= '<div class="dropdown dropdown-inline dd3-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right " x-placement="top-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-149px, -189px, 0px);">
                                    <a  data-id="' . $noeud->id_menu . '" class="dropdown-item edit" href="/' . CI_SITE_AREA . '/settings-advanced/' . $table . '/edit/' . $noeud->id_menu . '"><i class="kt-nav__link-icon flaticon2-contract"></i>' . lang('Core.edit') . '</a>
                                    <a  data-id="' . $noeud->id_menu . '" class="dropdown-item delete" href="/' . CI_SITE_AREA . '/settings-advanced/' . $table . '/delete/' . $noeud->id_menu . '"><i class="kt-nav__link-icon flaticon2-trash"></i>' . lang('Core.delete') . '</a>
                                </div>
                            </div>';
                $html .= "</div>";

                $niveau_precedent = $noeud->depth;
                $html .= afficher_menu_admin($noeud->id_menu, ($noeud->depth + 1), $array);
            }
        }
